...
class Thumbnail {
     ...
     * @return Thumbnail
     */
    public function __construct($fileName, $fileMime) {
        ...
        if($this->error == false) {
            //check if gif
            if(stristr(strtolower($this->fileMime),'gif')) $this->format = 'GIF';
            //check if jpg
            elseif(stristr(strtolower($this->fileMime),'jpg') || stristr(strtolower($this->fileMime),'jpeg')) $this->format = 'JPG';
            //check if png
            elseif(stristr(strtolower($this->fileMime),'png')) $this->format = 'PNG';
            //unknown file format
            else {
    ...
    private function calcImageByLargerSize($width,$height) {
        $newSize = array('newWidth'=>$width,'newHeight'=>$height);
     ...
     * @param int $maxHeight
     */
    public function resize($by_larger = false) {
        //$this->maxWidth = $maxWidth;
        //$this->maxHeight = $maxHeight;

        if($by_larger)
        	$this->calcImageByLargerSize($this->currentDimensions['width'],$this->currentDimensions['height']);
        else
	 ...
	 * @param int $cropSize
	 */
	public function cropFromCenter($cropWidth, $cropHeight) {
		$this->maxWidth = $cropWidth;
		$this->maxHeight = $cropHeight;
		$this->resize(true);
		
	    if($cropWidth > $this->currentDimensions['width']) $cropWidth = $this->currentDimensions['width'];
	    if($cropHeight > $this->currentDimensions['height']) $cropHeight = $this->currentDimensions['height'];
	...
	public function show($quality=100,$name = '') {
	    ...
	    switch($this->format) {
	        case 'GIF':      header('Content-Type: image/jpeg'); 
	            ImageGif($this->newImage,$this->cacheName);
	            break;
	        case 'JPG':
	        header('Content-Type: image/jpeg');
	            ImageJpeg($this->newImage,$this->cacheName,$quality);
	            break;
	        case 'PNG':   header('Content-Type: image/jpeg'); 
	            ImagePng($this->newImage,$this->cacheName);
	            break;
	    }
	    header('Content-Type: image/jpeg');
	    //exit;
	    //header('location: '.$this->cacheName);
	}
    ...
    public function buildCacheName(){
    	if($this->maxWidth && $this->maxHeight)
    		$filename = basename($this->fileName).'_'.$this->maxWidth.'_'.$this->maxHeight.'.'.$this->format;
    	if($this->cropWidth && $this->cropHeight)
    		$filename = basename($this->fileName).'_'.$this->cropWidth.'_'.$this->cropHeight.'_crop.'.$this->format;
		$this->cacheName = $this->cacheDir.'/'.$filename;
    }
    ...
    public function showThumb(){
    	...
    	if($this->checkIfCached()==false){
    		if($this->maxWidth && $this->maxHeight)
    			$this->resize();
    		elseif($this->cropWidth && $this->cropHeight)
    			$this->cropFromCenter($this->cropWidth, $this->cropHeight);
    		$this->show();
    	}
    }
